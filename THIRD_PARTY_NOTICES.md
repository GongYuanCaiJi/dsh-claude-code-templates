...
移植的子树与上游对应子树**逐字节相同**（本机验证 `diff -r` 全绿，排除「已移除」一节所列的 13 个 Anthropic
source-available 目录），并以 [`THIRD_PARTY_NOTICES.sha256`](./THIRD_PARTY_NOTICES.sha256)（5044 个文件的 SHA-256）
钉住，任何人可自验：
...
# 方式二：对照上游 pinned commit 直接 diff（排除已移除的 13 个 ARR 目录）
...
diff -r --exclude=document-skills --exclude=docx --exclude=docx-official --exclude=pdf --exclude=pdf-anthropic --exclude=pdf-official --exclude=pptx --exclude=pptx-official --exclude=xlsx --exclude=xlsx-official skills /tmp/upstream/cli-tool/components/skills
...
| 本 repo | 上游 |
|---|---|
| `skills/` | `cli-tool/components/skills/`（884 个 SKILL.md = 上游 897 − 13 个已移除的 ARR + 附属资源 + `ANTHROPIC_ATTRIBUTION.md`） |
...
除此之外：技能内容、目录结构、附属资源（references/、examples/、templates/ 等 4160 个文件）全部原样。

## 重复名称说明

上游有 21 个技能名跨类别重复（共 45 个文件；14 组内容逐字节相同、7 组内容不同）。dsh registry 以名称为键，
...
`skills/` 与 `.claude-plugin/` 不是统一授权，**以各目录自己的 `LICENSE` 文件为准**。全树 56 份目录级
`LICENSE` 文件分两类（均已原样保留，未改动）：
...
### Anthropic source-available（13 份，**已移除，不再随本包散布**）

上游这 13 个目录自带 Anthropic 版权声明（`© 2025 Anthropic, PBC. All rights reserved`），其 LICENSE.txt 明文禁止
Reproduce / Distribute / sublicense / transfer 给任何第三方（原文与 anthropics/skills 官方 repo 的 LICENSE.txt
逐字相同）。上游公开托管 ≠ 授权我们散布 —— 每个散布者独立负责。它们原样随上游搬入后，本 repo 再散布即违反条款，
因此已整体移除：
...
需要这些技能请直接去上游取用：[anthropics/skills](https://github.com/anthropics/skills)（官方源）或
[davila7/claude-code-templates](https://github.com/davila7/claude-code-templates)（本 repo 的上游）。上游把这批技能
标为 source-available / reference only 的完整说明原样保留在 `skills/ANTHROPIC_ATTRIBUTION.md`。
...
- 上游技能含 Anthropic 官方技能（`cli-tool/components/skills/ANTHROPIC_ATTRIBUTION.md` 有完整授权说明，
  原样保留，未删除）。其中 Apache-2.0 部分随包保留；source-available 的 13 个文档技能因 LICENSE 禁止再散布
  已移除（见上方「已移除」一节）。技能树整体为混合授权，逐目录明细见上方「技能树授权明细（按目录）」。
